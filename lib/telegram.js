...
class Telegram {
    ...
    _runMenu(menuData) {
        ...
        var ignoredKeys = ['message', 'layout', 'options', 'resize_keyboard']
        ...
        var options = {
            reply_markup: JSON.stringify({
                hide_keyboard: true,
                resize_keyboard: (menuData.resize_keyboard && menuData.resize_keyboard == true) ? true : false,
                one_time_keyboard: true,
                keyboard: keyboard
            })
        }
        ...
        if (menuData.options) {
            var keys = Object.keys(menuData.options)

            keys.forEach(key => options[key] = menuData.options[key])
        }
