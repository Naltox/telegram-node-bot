...
class Telegram {
    ...
    sendPhoto(chatId, photo, options, callback) {
        var preparedOptions =  this._prepareOptions(options, callback, { chat_id: chatId })
    ...
    sendMessage(chatId, text, options, callback) {
        var preparedOptions =  this._prepareOptions(options, callback, {chat_id: chatId, text: text})
    ...
    sendLocation(chatId, latitude, longitude, options, callback) {
        var preparedOptions = this._prepareOptions(options, callback, {chat_id: chatId, latitude: latitude, longitude: longitude })
    ...
    getUserProfilePhotos(userId, options, callback) {
        var preparedOptions = this._prepareOptions(options, callback, {user_id: userId})
    ...
    _processCommand(update) {
        ...
        if (this.waitingCallbacks[chatId] !== null && this.waitingCallbacks[chatId] !== undefined) {
        ...
        for (var routeIndex in this.router.routes) {
            ...
            for (var commandIndex in route.commands) {
                ...
                if (message !== undefined && message.indexOf(command) > -1) {
                    ...
                    if (this.controllersCommands[route.controller] !== undefined) {
                        for (var contextCommandIndex in this.controllersCommands[route.controller]) {
                            if (message !== undefined && message.indexOf(this.controllersCommands[route.controller][contextCommandIndex].command) > -1) {
                    ...
                    if (this.router.routes['otherwise'] !== undefined && commandIndex == route.commands.length - 1) {
    ...
    _prepareOptions(options, callback,  params){
        ...
        return { callback: callback, params: options }
    }
		    ...
		    this.waitForRequest(($) => {
			    formData[key].validator($.message, (valid) => {
				    if(valid == true){
					    ...
					    this.sendMessage(formData[key].error, { disable_web_page_preview: true }, () => {
						    run()
					    })
				    }
			    })
        ...
        for (var routeIndex in this.router.routes) {
            ...
            for (var commandIndex in route.commands) {
                ...
                if (command == _command) {
                    ...
                    var empty = true
                    this.controllersCommands[route.controller].forEach((item, i) => {
