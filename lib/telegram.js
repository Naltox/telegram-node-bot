...
Telegram.prototype.sendPhoto = function(chatId, photo, caption, callback){ 
	...
	net.post({
		...
		query: { chat_id: chatId, caption: caption },
		multipart: data,
		json: true
	}, function(body, response, err){ 
		if (!err) {  
...
Telegram.prototype.sendMessage = function(chatId, text, keyboard, callback){  
	...
	if(text.length > 4096){ 
		...
		this.call('sendMessage', { chat_id: chatId, text: text, reply_markup: JSON.stringify(keyboard) }, callback) 
	}  
}
Telegram.prototype._getUpdates = function(callback, offset){ 
	...
	self.call('getUpdates', { timeout: 50, offset: offset }, function(updates, err){
		if(!err && updates !== undefined){
			if( updates["result"] != undefined){ 
				if( updates["result"].length !== 0 ){  
					self.getUpdates(callback, updates["result"][updates["result"].length - 1]["update_id"] + 1)
					callback(updates) 
				}
				else{ 
					self.getUpdates(callback, offset)
				} 
			} 			
		} 
	})
...
Telegram.prototype._processCommand = function(commandsList, update){
	...
	for(var command in commandsList) {  
		if(message !== undefined && message.indexOf(command) > -1){  
			commandsList[command](chatId, userId, message.replace(command, '').trim(), update.message['reply_to_message'])
			return
		}     
    } 
}
Telegram.prototype.start = function(commandsList) {
	...
	self.getUpdates(function(updates) {
		self._asyncEach(updates, function(update) {
