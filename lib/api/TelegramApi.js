...
class TelegramApi {
    ...
    _handleRequest(request, resolve, reject) {
        req.post({
            ...
            if (body.error_code) {
                reject(TelegramApiError.fromResponse(body))
    ...
    _callWithInputFile(method, inputFile, type, params) {
        ...
        let sendedCallback = Function()

        return file.prepareRequest(type, params)
            .then(prepared =>  {
                sendedCallback = prepared.callback || Function()
    ...
    editMessageText(text, options) {
        ...
        return this.call('editMessageText', Object.assign(params, options))
            .then(r => typeof r.result == 'boolean' ? r.require : Message.deserialize(r))
    }
